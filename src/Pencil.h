...
private:
    ...
    vector<Element> vertices; // 多边形顶点和中心点
    ...
    void calAllpoints();      // 广搜算法算出所有点
    void calCoefficient();    // 全局插值法求所有点压力系数
    void calPressures();      // 计算各点承受的压力
    void calAvgPressures();   // 计算所有点的平均压力
//=============================================
    void triangulate();
    void rasterize(Element v1, Element v2, Element v3);
//=============================================
    int GRAPHITE_PERCENT;
    int CLAY_PERCENT;
    int WAX_PERCENT;

public:

    Pencil();                 // 中心及边界坐标和对应压力系数，
                              // 调用以下四个函数
